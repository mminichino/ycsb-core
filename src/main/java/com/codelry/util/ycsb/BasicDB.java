      ...
      synchronized (System.out) {
        ...
        if (p != null) {
          for (Enumeration e = p.propertyNames(); e.hasMoreElements();) {
            ...
            logger.debug("\"{}\"=\"{}\"", k, p.getProperty(k));
          }
        }
        logger.debug("**********************************************");
      }
  ...
  public void cleanup() {
    synchronized (MUTEX) {
      ...
      if (count && countDown < 1) {
        ...
        logger.debug("[READS], Uniques, {}", reads.size());
        logger.debug("[SCANS], Uniques, {}", scans.size());
        logger.debug("[UPDATES], Uniques, {}", updates.size());
        logger.debug("[INSERTS], Uniques, {}", inserts.size());
        logger.debug("[DELETES], Uniques, {}", deletes.size());
      }
    }
  }
